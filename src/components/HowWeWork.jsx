...
const steps = [
  { num:'01', phase:'Generate', title:'Paid media lands the prospect', body:'Google Ads capture high-intent search. Meta Ads build demand and retarget. Every campaign engineered to deliver a qualified prospect.', tags:['Google Ads','Meta Ads','Retargeting'] },
  ...
  { num:'03', phase:'Close', title:'Direct telesales converts', body:'This is where Zovek is different. We make the call. Consultative, persistent, built around a proven sales framework.', tags:['Telesales','Sales cycle','Closing'], highlight:true },
  { num:'04', phase:'Optimise', title:'Data drives the next cycle', body:'Close rates, objection patterns, cost per acquisition all fed back into the campaign layer. The system gets sharper with every deal.', tags:['Performance data','Optimisation','Compounding'] },
]
      ...
      <div className="max-w-[1280px] mx-auto">
        <div className="reveal mb-16">
          ...
          <h2 className="font-display font-black text-[#0a0a0a] tracking-[-1.5px] leading-[1.08]" style={{fontSize:'clamp(28px,3.5vw,46px)'}}>
            ...
            <br/>
            <span className="colour-grad-text">One team. One P&amp;L.</span>
          </h2>
          <p className="text-[#666] text-[16px] leading-relaxed max-w-[520px] mt-4">
            Most agencies stop at the lead. Zovek runs the whole race.
          </p>
        </div>
          ...
          {steps.map((s,i) => (
            <div key={s.num} className={`reveal reveal-delay-${i+1} relative rounded-2xl p-8 border overflow-hidden bg-white border-[#e8e8e8] hover:shadow-md transition-all duration-300`}>
              ...
              <div className="flex flex-wrap gap-2">
                {s.tags.map(t => (
                  <span key={t} className="text-[11px] px-2.5 py-1 rounded-full border border-[#e8e8e8] text-[#999]">{t}</span>
                ))}
              </div>
            </div>
          ))}
