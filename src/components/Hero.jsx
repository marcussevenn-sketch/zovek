...
export default function Hero() {
  ...
  const W=420, H=420
  const pos = id => { const n=NODES.find(n=>n.id===id); return [n.cx/100*W, n.cy/100*H] }
      ...
      <div className="max-w-[1280px] mx-auto px-8 w-full relative z-10 grid md:grid-cols-2 gap-12 items-center py-20">
        <div>
          <div className="hi reveal flex items-center gap-3 mb-10">
            ...
            <span className="text-[11px] font-medium tracking-[0.14em] uppercase text-[#999]">Revenue Infrastructure Company</span>
          </div>
          <h1 className="hi reveal font-display font-black leading-[1.0] tracking-[-3px] text-[#0a0a0a] mb-6"
          ...
          <div className="hi reveal flex flex-wrap gap-4 mb-16">
            ...
            <a href="#system" className="btn-outline-dark">See the system</a>
          </div>
          <div className="hi reveal grid grid-cols-3 gap-6 pt-8 border-t border-black/[0.07]">
            {[['4.2×','Avg ROAS'],['38%','↓ Cost per lead'],['90d','Full system live']].map(([n,l])=>(
              <div key={l}>
                <div className="font-display font-black text-[22px] tracking-[-1px] text-[#0a0a0a] leading-none mb-1">{n}</div>
          ...
          <div className="relative" style={{width:420,height:420,maxWidth:'100%'}}>
            <svg className="absolute inset-0 w-full h-full" viewBox={`0 0 ${W} ${H}`} fill="none">
              ...
              {EDGES.map(([a,b],i)=>{
                ...
                return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2}
                  stroke="url(#eg)" strokeWidth="1.5" strokeDasharray="5 5"
                  style={{animation:`flowDash ${2.5+i*0.35}s ease-in-out infinite`,animationDelay:`${i*0.5}s`}}/>
              })}
            ...
            <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none">
              <div className="text-center">
                <div className="font-display font-black text-[9px] tracking-[0.14em] uppercase text-[#bbb] mb-0.5">Zovek</div>
