...
export default function System() {
  ...
  return (
    <section id="system" ref={ref} className="bg-white py-24 md:py-36 px-8">
      <div className="max-w-[1280px] mx-auto">
        <div className="reveal mb-16">
          ...
          >
            Click to customer.<br />
            <span className="colour-grad-text">Fully owned.</span>
          </h2>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 items-start">
          <div className="flex flex-col gap-2">
            {STEPS.map((s, i) => (
              <div
                key={s.num}
                className={`reveal reveal-delay-${(i % 4) + 1} cursor-pointer rounded-2xl p-6 border transition-all duration-300 ${
                  active === i
                    ? 'bg-white border-[#e8e8e8] shadow-md'
                    : i % 2 === 0
                      ? 'bg-[#f7f7f5] border-transparent hover:border-[#e8e8e8]'
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  >
                    <span className="font-display font-black text-[10px]" style={{ color: s.colour }}>
                      {s.num}
                    </span>
                  </div>
                  <div className="flex-1">
                    ...
                    <span className="text-[11px] font-medium" style={{ color: s.colour }}>
                      {s.metric}
                    </span>
                  </div>
                  <svg
                    className={`w-4 h-4 text-[#ccc] transition-transform duration-300 ${active === i ? 'rotate-90' : ''}`}
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    viewBox="0 0 24 24"
          ...
          <div className="hidden md:block sticky top-24">
            <div className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-2xl p-8 relative overflow-hidden">
              ...
              <div className="font-display font-black text-[10px] tracking-[0.12em] uppercase text-[#666] mb-6">
                Revenue flow
              </div>
              {STEPS.map((s, i) => (
                <div key={s.num} className="flex items-start gap-3 mb-4 last:mb-0">
                  <div className="flex flex-col items-center">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${active === i ? 'shadow-md scale-110' : ''}`}
                      style={{ background: active === i ? s.colour : s.colour + '20' }}
                    >
                      <span className="font-display font-black text-[9px]" style={{ color: active === i ? '#fff' : s.colour }}>
                        {s.num}
                      </span>
                    </div>
                    {i < STEPS.length - 1 && (
                      <div className="w-px h-6 mt-1" style={{ background: active > i ? s.colour + '60' : '#2a2a2a' }} />
                    )}
                  </div>
                  <div className="pt-1.5">
                    <div
                      className={`font-display font-bold text-[12px] tracking-tight transition-colors duration-300 ${active === i ? 'text-white' : 'text-[#666]'}`}
                    >
                      {s.label}
                    </div>
                  </div>
