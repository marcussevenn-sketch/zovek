...
export default function NetworkDiagram() {
  ...
  const H = 500
  const pos = (id) => {
    const n = CNODES.find((node) => node.id === id)
    return [n.x / 100 * W, n.y / 100 * H]
  }

  return (
    <section ref={ref} className="bg-white py-24 md:py-36 px-8 border-y border-[#e8e8e8]">
      <div className="max-w-[1280px] mx-auto">
        <div className="reveal mb-16 text-center">
          ...
          <h2
            className="font-display font-black text-[#0a0a0a] tracking-[-1.5px] leading-[1.08] mb-4"
            style={{ fontSize: 'clamp(28px, 3.8vw, 48px)' }}
          >
            Everything connected.<br />
            <span className="colour-grad-text">Revenue at the centre.</span>
          </h2>
          ...
          <div className="relative" style={{ width: 500, height: 500, maxWidth: '90vw' }}>
            ...
            {CNODES.map((n, i) => (
              ...
              >
                {n.main ? (
                  <div
                    className="px-4 py-3 rounded-2xl shadow-xl node-pulse border border-[#1a1a1a]"
                    style={{ minWidth: 90, background: '#0a0a0a' }}
                  >
                    <div className="colour-grad-text font-display font-black text-[11px] tracking-tight">Revenue</div>
                    <div className="text-[8px] text-[#666] mt-0.5">The outcome</div>
                  </div>
                ) : (
                  <div
                    className="bg-white border border-[#e8e8e8] rounded-2xl px-2.5 py-2 shadow-sm hover:shadow-md transition-all duration-200 cursor-default node-pulse"
                    style={{ minWidth: 72, animationDelay: `${i * 0.4}s` }}
                  >
                    <div className="font-display font-black text-[8px] text-[#0a0a0a] tracking-tight whitespace-nowrap">
                      {n.label}
                    </div>
                  </div>
                )}
